#ifndef SOFIE_ROPERATOR_Erf
#define SOFIE_ROPERATOR_Erf
...
public:
   ...
   std::string Generate_GPU_Kernel_ALPAKA(std::string /*opName*/) override {
      ...
      op += SP + SP + SP + SP + "out[idx] = std::erf(data[idx]);\n";
      op += SP + SP + "}\n";
      op += SP + "}\n";
      op += "};\n";
      return op;
   ...
   std::string Generate_GPU_Kernel_Definitions_ALPAKA(std::string /*opName*/) override {
      return "ErfKernel erfKernel;\n";
   }

    std::string Generate_GPU_ALPAKA(std::string OpName) override {
      OpName = "op_" + OpName;
      if (fShape.empty()) {
         throw std::runtime_error("TMVA SOFIE Operator Erf called to Generate without being initialized first");
      }
      std::stringstream out;
      ...
      out << SP << "auto const workDiv_" << fNX << " = alpaka::getValidWorkDiv(kernelCfg_" << fNX << ", devAcc, erfKernel, alpaka::getPtrNative(deviceBuf_" << fNX
         << "), alpaka::getPtrNative(deviceBuf_" << fNY << "), static_cast<Idx>(" << length << "));\n";
      out << SP << "alpaka::exec<Acc>(queue, workDiv_" << fNX
         << ", erfKernel, alpaka::getPtrNative(deviceBuf_" << fNX
         << "), alpaka::getPtrNative(deviceBuf_" << fNY << "), static_cast<Idx>(" << length << "));\n";
      return out.str();
   }
